...
当前生产部署统一使用根目录 Docker Compose；数据库、Redis 与三个应用服务均由 Compose 编排：

```bash
git clone https://github.com/zengbbxx11/full-stack-songdian.git && cd full-stack-songdian
cp .env.example .env                 # 填写数据库口令、JWT、管理员密码和公网地址
docker compose build                 # 后端按 uv.lock、前后台按 package-lock.json 构建
docker compose up -d                 # 自动迁移并按健康依赖顺序启动五个服务
docker compose ps
...
- **质量门槛**：`.github/workflows/ci.yml` 在 PR 和 master push 时运行后端 Ruff/pytest、两个 Next.js 项目的 lint/typecheck/build
