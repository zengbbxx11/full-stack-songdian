...
> 当前状态（2026-09-16）：产品与新闻编辑已接入草稿、定时/立即发布、版本历史、恢复和 15 分钟短期预览；产品编辑页新增「商品详情图」区块（多图上传、说明、排序、移除）；询盘 CRM、通知、用户、设置和审计页面也已接入后端 API。媒体库支持查看图片引用位置和内容归档提示。当前部署与迁移说明以根目录 [`CURRENT_IMPLEMENTATION.md`](../CURRENT_IMPLEMENTATION.md) 为准。
...
商品详情图在产品编辑页维护，保存产品后生效并由官网产品详情页按顺序纵向展示；该区块的编辑结果写回产品的 `content_html`，因此与版本历史和发布缓存刷新复用同一条链路，不需要新增数据库结构。
...
│   │   ├── form/                 # 表单组件（含 ProductDetailImageEditor 商品详情图编辑器）
│   │   └── ui/                   # 基础 UI 组件
│   ├── layout/
...
| 产品管理 | 搜索/分类筛选、拖拽排序、新增/编辑/删除，**商品详情图**编辑器维护详情图片（多选/拖拽上传、逐张进度、说明与排序、移除二次确认；正文文字保留在数据中，表单内不再编辑正文文字） |
| 新闻管理 | 拖拽排序、新增/编辑/删除，**HTML 代码编辑器**编辑正文（右侧实时沙箱预览，支持**从媒体库插入图片**并自动写入 alt 与原始宽高），**发布时间编辑** |
...
| 媒体管理 | 图片上传、Products/News 子相册归档（侧边栏显示含子相册的合计数量）、相册新建/编辑（父级可选层级路径）与同级拖动排序、查看引用位置、删除风险提示、复制 URL |
| 用户管理 | 用户列表、创建、删除和重置密码 |
| 系统设置 | 站点设置、询盘 SMTP 配置与**首页轮播**（3 槽位：图片/手机端竖版图/启用/跳转链接），支持普通字段回显、敏感授权码脱敏和测试发送 |
...
- **首页轮播**（页面底部「首页轮播」分组）：固定 3 个槽位，每槽可选桌面图 + 手机端专用竖版图、启用开关和跳转链接。第 1 槽是首屏主图 —— 留空即官网默认 Banner（保留原有悬浮文字与按钮），所以它没有启用开关、链接框禁用；第 2、3 槽是纯图轮播。该分组**独立保存**，不参与上面的普通字段差量提交。产图标准见分组内折叠说明（桌面 1920×1080 ≤500KB；手机竖版 1080×1350 或 1080×1920 ≤350KB；主体居中留 10% 安全边距；第 1 张左侧 60% 不放关键主体）。
...
- 相册树交互（2026-09-22）：点「+」时父级默认是**当前正在浏览的相册**，创建成功后自动选中新相册并展开其祖先；编辑时父级下拉按**层级路径**展示并隐藏自身与全部子孙（挂上去会成环），同级重名只给行内提示、不阻断创建。相册弹窗已**移除排序数字输入**，改为桌面端**同级拖动重排**（跨父级拖动忽略）与上移/下移，落库走 `PUT /admin/albums/sort`（数组下标即 `sort_order`）。口径细节见 [`AGENTS.md`](./AGENTS.md) 的「媒体库相册交互与排序（2026-09-22）」。
...
- 产品编辑页的「商品详情图」由 `src/components/form/ProductDetailImageEditor.tsx` 承担：用 `DOMParser` 把 `content_html` 中的 `img` 摘出为可编辑列表，编辑完成后按同结构序列化回 HTML。原有文字仍保留在数据中，官网详情区只展示图片。
- 上传时先用 `createImageBitmap()` 读取原始宽高再上传，官网据此预留正确比例；上传期间通过 `onBusyChange(true)` 禁用保存按钮与表单字段，避免半成品内容被保存。
- 支持为每张图填写说明（写入 `alt`）、上移/下移调整顺序、逐张移除；封面与图库仍是独立字段，不受该区块影响，图库与规格的增删仍为立即保存。
- 详情图保存在产品正文中，复用既有上传媒体、版本历史与发布缓存刷新链路；草稿预览页面使用相同渲染规则。
- 交互（2026-09-17）：支持多选与**拖拽上传**、**逐张进度**（`正在上传第 x/y 张…`）、移除前**二次确认**、以及无图时的空态引导；详情图有改动未保存时，刷新/关闭页面会被 `beforeunload` 提示，点「取消」会先弹确认（保存成功后自动复位）。
- 新闻正文编辑器（`components/form/RichTextEditor.tsx`）是**纯 HTML 代码编辑器**：代码框直接写/粘 HTML，「从媒体库插入图片」读取原图宽高后在代码光标处插入 `<img src alt width height>`，上传期间禁用保存；右侧实时预览先按入库白名单清洗，再渲染进 `sandbox=""` 的 iframe（脚本无法执行），最终外观以官网为准。
- 建议详情图宽度 1200–1600 px、优先 WebP、超长图分段上传；图片内容由运营提供，后台不做压缩或裁剪。
