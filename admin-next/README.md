...
| 新闻管理 | 拖拽排序、新增/编辑/删除，**富文本编辑器**编辑内容（支持**插入图片**，自动写入 alt 与原始宽高），**发布时间编辑** |
...
- 新闻正文编辑器（`components/form/RichTextEditor.tsx`）新增**插入图片**：点击工具栏图片按钮后上传（复用 `POST /admin/upload`），插入 `<img src alt width height>`，宽高取自原始尺寸供官网做响应式与懒加载；上传期间禁用保存。
