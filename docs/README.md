...
| `design-product-tags.md` | 产品标签恢复设计（WP tags → `Product.tags` JSONField 全链路）**（已作废：依赖的 M6 迁移模块已移除）** |
| `admin-ui-class-diagram.mermaid` | 后台管理类图（API 契约、模型关系、权限码） |
| `admin-ui-sequence-diagram.mermaid` | 后台管理时序图（登录鉴权流、CRUD 流、图片上传流） |
| `class-diagram-product-tags.mermaid` | 标签恢复类图（WP 适配器、ETL、模型 schema）**（基于已移除的 M6 模块，仅供参考）** |
| `sequence-diagram-product-tags.mermaid` | 标签恢复时序图（ETL 拉取 → 适配 → 写入 → 前端消费）**（基于已移除的 M6 模块，仅供参考）** |
