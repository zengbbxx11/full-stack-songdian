...
export async function generateMetadata({ searchParams }: { searchParams: Promise<ListSearchParams> }): Promise<Metadata> {
  ...
  const { category: slug, page: requestedPage } = readListQuery(sp);
  const { category: cat, pagination, failed, retryCategory } = await getProductsPage(requestedPage, slug);
  const outOfRange = !failed && requestedPage > (pagination?.totalPages || 1);
  const canonicalPage = outOfRange ? 1 : requestedPage;

  const canonicalUrl = listUrl("/products", canonicalPage, failed ? retryCategory : cat?.slug);
  const pageTitle = canonicalPage > 1 ? " | Page " + canonicalPage : "";
  ...
  const meta = cat
    ...
    : await superMeta({
        title: "Digital Camera Products for OEM & ODM" + pageTitle,
...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  const { page: currentPage, category: categorySlug } = readListQuery(params);

  const { categories: rawCategories, category: matchedCategory, items: products, pagination, failed: loadError, retryCategory } = await getProductsPage(currentPage, categorySlug);
        ...
        <div className="site-container">
          ...
          {loadError ? (
            <div className="text-center py-24 bg-gray-50 border border-[var(--border)]" style={{ borderRadius: "12px" }}>
              ...
              <a
                href={listUrl("/products", currentPage, retryCategory)}
